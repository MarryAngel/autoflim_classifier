...
def rename_and_move_results(random_state, nimage, superpixel, nfeat, technique):
    ...
    target_dir = os.path.join(base_dir, dataset_name, 'experiments', 'prototypeEvaluation_filterReduction', 'results', f'super{superpixel}_{nimage}images_{technique}')
...
def experiment_pipeline(random_state, technique, distance_flag, nimages_final, superpixel, nclass, split, last_layer, output_dir):
    ...
    run_script('train_flim_svm_filter_reduction.py', 
                          f"{dataset_name} {last_layer} {split} {superpixel}", 
                          f"SVM training with filter reduction (superpixel={superpixel})")
