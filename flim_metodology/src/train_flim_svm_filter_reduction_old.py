...
# Create layer directories and flim directory with filter reduction
for i in range(1, layer + 1):
    if i == 1:
        cmd = f"iftCreateSortedLayerModel images None bag arch2D.json {i} flim"
    else:
        cmd = f"iftCreateSortedLayerModel images layer{i-1} bag arch2D.json {i} flim"
    os.system(cmd)
...
# Para avaliar a performance em cada layer, descomentar o bloco abaixo
# for l in range(1, layer + 1):
#     # Move layerN to layerN_train{split}
#     cmd = f"mv layer{l} layer{l}_train{split}"
#     os.system(cmd)

#     # Create layerN_train{split}_id_image.txt and layerN_train{split}.zip
#     cmd = f"iftActivDataSet layer{l}_train{split} images 1 1"
#     os.system(cmd)

#     # Train SVM and create svm_{layer}_{split}.zip
#     cmd = f"iftSupTrainBySVM layer{l}_train{split}.zip 0 0 1e2 0 svm_{l}_{split}.zip"
#     os.system(cmd)