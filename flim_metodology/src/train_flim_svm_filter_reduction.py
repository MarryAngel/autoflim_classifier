...
if len(sys.argv) != 6:
    ...
    print("percent_bofp: Percentage of Bag of FPs to be used (float between 0 and 1)")
    exit()
...
cmd = f"rm -rf initial_bag bag flim layer[0-{layer}] layer{layer}_train{split} superpixels train.csv seeds_files.txt"
os.system(cmd)
...
cmd = "iftSeedsFromSuperpixels superpixels initial_bag 1"
os.system(cmd)

# Create seeds_files.txt with files in bag directory
cmd = "ls -v initial_bag >> seeds_files.txt"
os.system(cmd)
...
cmd = f"iftSelectBoFP images initial_bag {kernel_size} {dilation_rate} bag {percent_bofp}"
os.system(cmd)

# move file -scores.csv to bag for folder bag_scores
cmd = "mkdir -p bag_scores && mv -f bag/*.csv bag_scores/"
os.system(cmd) 
