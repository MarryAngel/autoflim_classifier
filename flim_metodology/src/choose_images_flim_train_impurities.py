...
def select_images(labels_to_images, num_classes, num_images_per_class, random_state):
    # Select images per class; last class gets double the total from other classes
    selected_images = []
    random.seed(random_state)
    total_other = num_images_per_class * (num_classes - 1)
    for class_idx in range(1, num_classes + 1):
        if class_idx in labels_to_images:
            if class_idx == num_classes:
                required = 2 * total_other
                if len(labels_to_images[class_idx]) < required:
                    print(f"Not enough images in class {class_idx} for selection.")
                    sys.exit(1)
                selected = random.sample(labels_to_images[class_idx], required)
            else:
                if len(labels_to_images[class_idx]) < num_images_per_class:
                    print(f"Not enough images in class {class_idx} for selection.")
                    sys.exit(1)
