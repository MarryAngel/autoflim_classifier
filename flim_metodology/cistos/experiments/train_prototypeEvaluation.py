...
def rename_and_move_results(random_state, nimage, superpixel, nfeat, technique):
    """Renames result files, adds nfeat, and moves them to the target directory."""

    results_dir = os.path.join(build_dir, 'results_3_1')
    target_dir = os.path.join(base_dir, dataset_name, 'experiments', 'prototypeEvaluation', 'results', f'super{superpixel}_{nimage}images_{technique}')
    ...
    os.rename(os.path.join(results_dir, 'layer3_test1-classified-images.csv'), os.path.join(results_dir, classified_img))
...
def experiment_pipeline(random_state, technique, distance_flag, nimages_final, superpixel, nclass, split, last_layer, output_dir):
    ...
    while nimages < nimages_final:
        run_script('iftPrototypeEvaluation', f"layer{last_layer}_train{split}.zip train.csv {output_dir} {distance_flag}", f"Prototype evaluation ({technique})")
        ...
        if nimages < nimages_final:
            ...
            try:
                ...
                nfeat = "unknown"
            rename_and_move_results(random_state, nimages, superpixel, nfeat, technique)
...
def main():
    ...
    techniques = [("cossine", 0), ("euclidean", 1)]
    total_experiments = len(random_indices) * len(techniques)
    experiments_done = 0

    print(f"🚀 Starting {total_experiments} experiments for Euclidean and Cosine distances")    
