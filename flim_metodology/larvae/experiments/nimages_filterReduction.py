...
def rename_and_move_results(random_state, nimage, superpixel, nfeat, reduction):
    """Rename result files, append nfeat, and move them to the target directory."""
    
    results_dir = os.path.join(build_dir, 'results_3_1')
    
    if not reduction:
        target_dir = os.path.join(base_dir, dataset_name, 'experiments', 'nimages_filterReduction', 'results', f'super{superpixel}_{nimage}images')
    else:
        target_dir = os.path.join(base_dir, dataset_name, 'experiments', 'nimages_filterReduction', 'results', f'super{superpixel}_{nimage}images_filterReduction')
    ...
    with tqdm(total=total, desc="Experiments", position=0, leave=True) as pbar:
        for i, nimage in enumerate(nimages):
            for j, random_state in enumerate(random_indices):
                ...
                rename_and_move_results(random_state, nimage, superpixel, nfeat, reduction)

                # Com redução dos filtros
                reduction = True
                # Run training script with filter reduction
                ...
                result_deploy = run_script('deploy_flim_svm.py',
                                         f"{dataset_name} {last_layer} {split}",
                                         "Deploy and evaluation with filter reduction") 
                # Extract nfeat from deploy output
                try:
                    nfeat = result_deploy.stdout.split("nfeats: ")[1].split(",")[0]
                except (IndexError, AttributeError):
