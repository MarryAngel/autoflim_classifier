...
def rename_and_move_results(random_state, superpixel, nfeat, reduction):
    """Rename result files, append nfeat, and move them to the target directory."""
    
    results_dir = os.path.join(build_dir, 'results_3_1')
    
    if not reduction:
        target_dir = os.path.join(base_dir, dataset_name, 'experiments', 'nsuperpixels_filterReduction', 'results', f'super{superpixel}')
    else:
        target_dir = os.path.join(base_dir, dataset_name, 'experiments', 'nsuperpixels_filterReduction', 'results', f'super{superpixel}_filterReduction')
    ...
    with tqdm(total=total, desc="Experiments", position=0, leave=True) as pbar:
        for i, superpixel in enumerate(superpixels):
            for j, random_state in enumerate(random_indices): 
                ...
                rename_and_move_results(random_state, superpixel, nfeat, reduction)
