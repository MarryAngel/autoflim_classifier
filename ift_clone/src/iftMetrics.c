...
float iftCosineDistance2(float *f1, float *f2, int n) {
    ...
    v1_norm = sqrtf(v1_norm) + 1e-12;
    v2_norm = sqrtf(v2_norm) + 1e-12;
...
float iftCrossCorrCoefDist(float *x, float *y, int n)
{
    if (n <= 1) return 1.0f;  // Invalid input
    
    // Calculate means and standard deviations
    float mean1 = iftMean(x, n);
    float mean2 = iftMean(y, n);
    
    float std1 = iftStd(x, n);
    float std2 = iftStd(y, n);

    if (std1 == 0.0f || std2 == 0.0f) {
        return 1.0f;  // No variation in one or both signals
    }
    
    // Find maximum normalized cross-correlation across all possible lags
    float max_correlation = -1.0f;
    
    // Check all possible lags (shifts)
    for (int lag = -(n-1); lag < n; lag++) {
        ...
        for (int i = 0; i < n; i++) {
            int j = i + lag;
            if (j >= 0 && j < n) {
                correlation += ((x[i] - mean1) / std1) * ((y[j] - mean2) / std2);
                count++;
            }
        }
        
        if (count > 0) {
            correlation /= count;  // Normalize by number of overlapping points
    ...
    // Return cross-correlation distance: 1 - max_correlation
    return 1.0f - max_correlation;
}
