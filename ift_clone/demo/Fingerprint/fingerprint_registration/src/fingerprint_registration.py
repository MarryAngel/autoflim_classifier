...
def main():
    ...
    if len(sys.argv) != 6:
        ...
        message += 'P2: input folder with all fingerprint images (.png)\n'
        message += 'P3: input folder with all minutia files (.txt)\n'
        message += 'P4: input folder with all matches (.txt)\n'
        message += 'P5: output folder with registered images and score list\n'
    ...
    input_matches_folder   = sys.argv[4]

    # creating output directories and result list file
    experiment_directory        = '../outputs/' + sys.argv[5]
    ...
    for i, line in enumerate(input_comparisons_file):
        ...
        # selecting matches file
        input_matches_file = (input_matches_folder + latent.replace('.png', '') + '_' + reference.replace('.png', '') + '.txt')
        
        # calculating parameters of similarity transformation from minutia points
        scale, theta, tx, ty, inliers = candidate_based_ransac_similarity_transformation(latent_mnts, reference_mnts, input_matches_file)
        if scale == -1:
            register_result_list.write(latent + ' ' + reference + ' 0\n')
            print('Algorithm failed in step {}'.format(i+1))
            continue
        register_result_list.write(latent + ' ' + reference + ' 1\n')
