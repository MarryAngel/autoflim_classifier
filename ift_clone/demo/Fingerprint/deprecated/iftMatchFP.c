...
typedef struct _fp_match_problem {
  ...
  float      mean, stdev; /* statistics of the source for normalization */
} FPMatchProblem;

FPMatchProblem *CreateFPMatchProblem(iftImage *source, iftImage *target, char *source_basename, char *target_basename);
				     ...
				     char *target_basename)
{
  ...
  for (int p=0; p < source->n; p++)
    fpmp->mean += source->val[p];
  fpmp->mean /= source->n;
  
  fpmp->stdev = 0.0;
  for (int p=0; p < source->n; p++)
    fpmp->stdev += (source->val[p] - fpmp->mean)*(source->val[p] - fpmp->mean);
  fpmp->stdev = sqrt(fpmp->stdev/source->n) + 0.001;
...
float FPMatchFitness(void *problem, float *theta)
{
  ...
  while (S != NULL) {
    ...
    if (iftValidVoxel(fpmp->target,v)){
      int q = iftGetVoxelIndex(fpmp->target,v);
      cost += fabs((fpmp->target->val[q]-fpmp->mean)/fpmp->stdev-(fpmp->source->val[p]-fpmp->mean)/fpmp->stdev);
    } else {
      cost += 10.0;//fpmp->Imax;
    }
    S = S->next;
...
int main(int argc, char **argv) {
  ...
  if (argc != 5) {
    printf("iftMatchFP <P1> <P2> <P3> <P4>\n");
    printf("P1: folder with all images\n");
