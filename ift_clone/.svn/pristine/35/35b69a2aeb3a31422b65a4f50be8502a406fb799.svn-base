...
#define SIMILARITY_THRESHOLD 0.59
...
double spatialSimilarityScoreUsingAnchor(int latent_mnt_id, int reference_mnt_id, int latent_anchor, int reference_anchor, MinutiaPoint *latent_mnts, MinutiaPoint *reference_mnts, iftMatrix *similarity_matrix){
  

    MinutiaPoint latent_segment[2]      = { latent_mnts[latent_mnt_id], latent_mnts[latent_anchor] };
    MinutiaPoint reference_segment[2]   = { reference_mnts[reference_mnt_id], reference_mnts[reference_anchor] };
    double score                        = segmentSimilarityScore(latent_segment, reference_segment, latent_mnts, reference_mnts, similarity_matrix);

    return score;
}

MntPair findGlobalAnchor(iftMatrix *similarity_matrix, int n_latent_mnts, int n_reference_mnts, MinutiaPoint *latent_mnts, MinutiaPoint *reference_mnts){
    float *flattened_matrix;
    int *idxs;

    initializeScoresArray(similarity_matrix->n, &flattened_matrix, &idxs);
    for (int i = 0; i < similarity_matrix->n; i++) flattened_matrix[i] = similarity_matrix->val[i];

    iftFQuickSort(flattened_matrix, idxs, 0, similarity_matrix->n - 1, IFT_DECREASING);

    MntPair *anchor = (MntPair *)calloc(K_ANCHOR, sizeof(MntPair));
    for (int i = 0; i < K_ANCHOR; i++){
        anchor[i].latent_id    = idxs[i] / similarity_matrix->ncols;
        anchor[i].reference_id = idxs[i] % similarity_matrix->ncols;
        anchor[i].score        = flattened_matrix[i];
        anchor[i].score        *= spatialSimilarityScore(anchor[i].latent_id, anchor[i].reference_id, n_latent_mnts, n_reference_mnts, latent_mnts, reference_mnts, similarity_matrix);
    }



    float max   = IFT_INFINITY_DBL_NEG;
    int idx_max = 0;

    for (int i = 0; i < K_ANCHOR; i++){
        if (anchor[i].score > max){
            max     = anchor[i].score;
            idx_max = i;
        }
    }
    ...
    MntPair final_anchor;
    final_anchor.latent_id    = anchor[idx_max].latent_id;
    final_anchor.reference_id = anchor[idx_max].reference_id;
    final_anchor.score        = anchor[idx_max].score;
...
iftMatrix *calculateSpatialCoherentSimilarityMatrix(iftMatrix *similarity_matrix, int n_latent_mnts, int n_reference_mnts, MinutiaPoint *latent_mnts, MinutiaPoint *reference_mnts){
    ...
    for (int i = 0; i < similarity_matrix->nrows; i++){
        for (int j = 0; j < similarity_matrix->ncols; j++){
            if (iftMatrixElem(similarity_matrix, j, i) != 0){
                ...
                iftMatrixElem(spatial_coherent_similarity_matrix, j, i) = 0;
            }
...
iftMatrix *calculateSpatialCoherentSimilarityMatrixUsingAnchor(iftMatrix *similarity_matrix, int n_latent_mnts, int n_reference_mnts, MinutiaPoint *latent_mnts, MinutiaPoint *reference_mnts, MntPair anchor){
    iftMatrix *spatial_coherent_similarity_matrix = iftCopyMatrix(similarity_matrix);
    for (int i = 0; i < similarity_matrix->nrows; i++){
        if (i == anchor.latent_id) continue;
        for (int j = 0; j < similarity_matrix->ncols; j++){
            if (j == anchor.reference_id) continue;
            if (iftMatrixElem(similarity_matrix, j, i) != 0){
                double spatial_similarity_score = spatialSimilarityScoreUsingAnchor(i, j, anchor.latent_id, anchor.reference_id, latent_mnts, reference_mnts, similarity_matrix);
                iftMatrixElem(spatial_coherent_similarity_matrix, j, i) += spatial_similarity_score;
                iftMatrixElem(spatial_coherent_similarity_matrix, j, i) /= 2.0;
            }
            else{
    ...
    for (int c = 0; c < similarity_matrix->ncols; c++){
        iftMatrixElem(spatial_coherent_similarity_matrix, c, anchor.latent_id) = 0;
    }

    for (int r = 0; r < similarity_matrix->nrows; r++){
        iftMatrixElem(spatial_coherent_similarity_matrix, anchor.reference_id, r) = 0;
    }

    iftMatrixElem(spatial_coherent_similarity_matrix, anchor.reference_id, anchor.latent_id) = anchor.score;
    return spatial_coherent_similarity_matrix;
}
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < query_fs->n; i++){
        ...
        for (int j = 0; j < ref_fs->n; j++){
            ...
            MntPair    anchor                             = findGlobalAnchor(similarity_matrix, n_latent_mnts, n_reference_mnts, latent_mnts, reference_mnts);
            iftMatrix *spatial_coherent_similarity_matrix = calculateSpatialCoherentSimilarityMatrixUsingAnchor(similarity_matrix, n_latent_mnts, n_reference_mnts, latent_mnts, reference_mnts, anchor);
