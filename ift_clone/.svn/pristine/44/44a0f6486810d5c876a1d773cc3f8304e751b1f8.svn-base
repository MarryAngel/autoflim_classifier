...
class FingerprintImageEnhancer(object):
    ...
    def __ridge_segment(self, img):
        ...
        # print("mask =", np.round(self._mask*255))

        cv2.imwrite(f"/data_lids/home/gabrielseabra/ift/demo/Fingerprint/exs_output/mask_{random.randint(0, 10000)}.png", np.round(self._mask*255))
    ...
    def __ridge_orient(self):
        ...
        orient_vis = np.empty_like(self._orientim)
        cv2.normalize(self._orientim, orient_vis, 0, 255, cv2.NORM_MINMAX)
        cv2.imwrite(f"orientations{random.randint(0, 10)}.png", orient_vis)
    ...
    def __ridge_freq(self):
        ...
        self._freq  = np.ones_like(self._mean_freq) * self._mask * 0.125
    ...
    def enhance(self, img, resize=False):
        ...
        enhanced_img = self.__ridge_filter()       # filter the image using oriented gabor filter
        return(enhanced_img, self._binim * 255)
